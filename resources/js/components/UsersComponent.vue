<template>
  <div>
    <v-col cols="12" sm="6" class="mb-5">
      ...
      <v-btn-toggle v-model="selectedUser" shaped mandatory>
        <v-btn value=""> All </v-btn>
        <v-btn value="admin"> Admin </v-btn>
        <v-btn value="teacher"> Teacher </v-btn>
...
export default {
  ...
  methods: {
    initialize() {
      let param = `?r=${this.selectedUser}`;
      if (!this.selectedUser) param = "";

      axios.get(`${this.$baseUrl}/api/user${param}`).then((response) => {
